...
% % s is the Generalized Hausdorff pseudo-similarity of the fpfs-matrices a and b 
% a=rand(5,4); 
% b=rand(5,4); 
% s=fpfss8(a,b,3);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Generalized Hausdorff pseudo-similarity over fpfs-matrices
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function X=fpfss8(a,b,p)
if size(a)~=size(b)
...
[m,n]=size(a);
  ...
  for i=2:m
    for j=1:n
       d(j)=abs(a(1,j)*a(i,j)-b(1,j)*b(i,j))^p;
    end
    e(i-1)=max(d);
  end
  X=1-(sum(e)^(1/p)/((m-1)^(1/p)));
end
end